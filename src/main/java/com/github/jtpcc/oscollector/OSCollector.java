...
package com.github.jtpcc.oscollector;
...
public class OSCollector {
	...
	public OSCollector(String script, int runID, int interval,
		...
		try {
			...
			resultCSVs[0] = new BufferedWriter(
					new FileWriter(new File(outputDir, "sys_info.csv")));
	...
	private class CollectData implements Runnable {
		...
		public void run() {
			...
			osData = new BufferedReader(
					new InputStreamReader(parent.collProc.getInputStream()));
