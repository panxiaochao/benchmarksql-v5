...
public class jTPCCTerminal implements jTPCCConfig, Runnable {
    ...
    public jTPCCTerminal(String terminalName, int terminalWarehouseID, int terminalDistrictID,
                         ...
                         int paymentWeight, int orderStatusWeight, int deliveryWeight, int stockLevelWeight,
    ...
    private void executeTransactions(int numTransactions) {
        ...
        for (int i = 0; (i < numTransactions || numTransactions == -1) && !stopRunning; i++) {
            ...
            /*
             * TPC/C specifies that each terminal has a fixed "home" warehouse. However, since this
             * implementation does not simulate "terminals", but rather simulates "application threads",
             * that association is no longer valid. In the case of having less clients than warehouses
             * (which should be the normal case), it leaves the warehouses without a client without any
             * significant traffic, changing the overall database access pattern significantly.
             */
            ...
            if (transactionType <= paymentWeight) {
                ...
                try {
                    ...
                    /*
                     * The old style driver does not have a delivery background queue, so we have to execute
                     * that part here as well.
                     */
