...
public class jTPCCTData {
	...
	public void traceScreen(Logger log) throws Exception {
		...
		synchronized (traceLock) {
			fmt.format(
					...
					(transEnd == 0) ? "INPUT" : "OUTPUT", terminalWarehouse,
					terminalDistrict);
			sb.setLength(79);
	...
	private void executeNewOrder(Logger log, jTPCCConnection db)
		 ...
		 */
		for (ol_cnt = 0; ol_cnt < 15
				&& newOrder.ol_i_id[ol_cnt] != 0; ol_cnt++) {
		...
		try {
			...
			if (!rs.next()) {
				rs.close();
				throw new SQLException("District for" + " W_ID=" + newOrder.w_id
			...
			for (int i = 0; i < ol_cnt; i++) {
				...
				if (item == null) {
					...
					throw new Exception(
							"ITEM " + newOrder.ol_i_id[seq] + " not fount");
				}
			}
			...
			while (rs.next()) {
				...
				for (int i = 0; i < ol_cnt; i++) {
					int seq = ol_seq[i];
					if (newOrder.ol_i_id[seq] == i_id
							&& newOrder.ol_supply_w_id[seq] == w_id) {
						newOrder.s_quantity[seq] = rs.getInt("s_quantity");
						newOrder.dist_value[seq] = rs.getString(distName);
						newOrder.found[seq] = true;
						if (item != null) {
							newOrder.ol_amount[seq] = item.i_price
									* newOrder.ol_quantity[seq];
							if (item.i_data.contains("ORIGINAL") && rs
									.getString("s_data").contains("ORIGINAL"))
			...
			for (int i = 0; i < 15; i++) {
				if (newOrder.ol_i_id[i] != 0)
					fmt[6 + i].format(
							...
							newOrder.ol_supply_w_id[i], newOrder.ol_i_id[i],
							newOrder.i_name[i], newOrder.ol_quantity[i],
							newOrder.s_quantity[i], newOrder.brand_generic[i],
	...
	private void executePayment(Logger log, jTPCCConnection db)
		...
		try {
			...
			rs = stmt.executeQuery();
			if (!rs.next()) {
				throw new Exception("Customer for" + " C_W_ID=" + payment.c_w_id
						+ " C_D_ID=" + payment.c_d_id + " C_ID=" + payment.c_id
						+ " not found");
			}
	...
	private void traceOrderStatus(Logger log, Formatter fmt[]) {
		...
		if (transEnd == 0) {
			...
			fmt[6].format(
					"Suppy-W      Item-Id     Qty    Amount        Delivery-Date");
		} else {
			// ORDER_STATUS OUTPUT screen
			fmt[1].format("Warehouse: %6d   District: %2d", orderStatus.w_id,
					orderStatus.d_id);
			fmt[2].format("Customer: %4d   Name: %-16.16s %2.2s %-16.16s",
					orderStatus.c_id, orderStatus.c_first, orderStatus.c_middle,
					orderStatus.c_last);
		...
		try {
			for (d_id = 1; d_id <= 10; d_id++) {
				...
				while (o_id < 0) {
					...
					rs.close();
					/*
					 * This logic only works in SNAPSHOT isolation level.
					 * Because we select new_order for update, the order must be
					 * not selected by other termnial as long as this SQL
					 * statement return it.
					 */
				}
			...
			/*
			 * We found out oldest undelivered order for this DISTRICT and the
			 * NEW_ORDER line has been deleted. Process the rest of the
			 * DELIVERY_BG.
			 */
			...
			for (d_id = 1; d_id <= 10; d_id++) {
				...
				if (o_id >= 0 && deliveryBG.delivered_c_id[d_id - 1] < 0) {
					throw new Exception("ORDER in DELIVERY_BG for" + " O_W_ID="
							+ deliveryBG.w_id + " O_D_ID=" + d_id + " O_ID="
							+ o_id + " not found");
				}
			}
			...
			while (rs.next()) {
				...
				deliveryBG.sum_ol_amount[d_id - 1] = rs
						.getDouble("sum_ol_amount");
			}
			rs.close();
			...
			for (d_id = 1; d_id <= 10; d_id++) {
				...
				if (ans < 0) {
					throw new Exception("sum(OL_AMOUNT) for ORDER_LINEs with "
							+ " OL_W_ID=" + deliveryBG.w_id + " OL_D_ID=" + d_id
