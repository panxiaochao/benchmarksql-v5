...
public class jTPCCTData {
  ...
  public final static String transTypeNames[] = {"NEW_ORDER", "PAYMENT", "ORDER_STATUS",
  ...
  private void executeDelivery(Logger log, jTPCCConnection db) {
    ...
    /*
     * The DELIVERY transaction is different from all the others. The foreground transaction,
     * experienced by the user, does not perform any interaction with the database. It only queues a
     * request to perform such a transaction in the background (DeliveryBG). We store that TData
     * object in the delivery part for the caller to pick up and queue/execute.
     */
  ...
  private class DeliveryData {
    ...
    /*
     * executeDelivery() will store the background request here for the caller to pick up and
     * process as needed.
     */
  ...
  private void generateDeliveryBG(int w_id, long due, String ol_delivery_d, jTPCCTData parent) {
    /*
     * The DELIVERY_BG part is created as a result of executing the foreground part of the DELIVERY
     * transaction. Because of that it inherits certain information from it.
     */
    ...
    try {
      for (d_id = 1; d_id <= 10; d_id++) {
        ...
        while (o_id < 0) {
          ...
          rs.close();
          /*
           * This logic only works in SNAPSHOT isolation level. Because we select new_order for
           * update, the order must be not selected by other termnial as long as this SQL statement
           * return it.
           */
        }
