...
import com.github.jtpcc.oscollector.OSCollector;
...
public class jTPCC implements jTPCCConfig {
	...
	public jTPCC() {
		...
		if (databaseDriverLoaded && resultDirectory != null) {
			...
			try {
				...
				runInfoCSV.write(
						"run,driver,driverVersion,db,sessionStart," + "runMins,"
			...
			if (osCollectorScript != null) {
				osCollector = new OSCollector(getProp(ini, "osCollectorScript"),
						...
						getProp(ini, "osCollectorDevices"), resultDataDir, log);
			}
		...
		if (databaseDriverLoaded) {
			try {
				...
				int newOrderWeightValue = -1, paymentWeightValue = -1,
						orderStatusWeightValue = -1, deliveryWeightValue = -1,
				...
				switch (dbType) {
					case DB_FIREBIRD :
						/*
						 * Firebird needs no_rec_version for our load to work.
						 * Even with that some "deadlocks" occur. Note that the
						 * message "deadlock" in Firebird can mean something
						 * completely different, namely that there was a
						 * conflicting write to a row that could not be
						 * resolved.
						 */
				...
				try {
					loadWarehouses = Integer.parseInt(
							jTPCCUtil.getConfig(iConn, dbProps, "warehouses"));
					CLoad = Long.parseLong(
							jTPCCUtil.getConfig(iConn, dbProps, "nURandCLast"));
				} catch (Exception e) {
				...
				if (Long.parseLong(iRunMins) != 0
					...
					try {
						...
						errorMessage(
								"Invalid number of transactions per terminal!");
						throw new Exception();
					}
				}
				...
				if (newOrderWeightValue + paymentWeightValue
					...
					errorMessage(
							"Sum of mix percentage parameters exceeds 100%!");
					throw new Exception();
				}
				...
				if (!limitIsTime)
					...
					printMessage(
							"Creating " + numTerminals + " terminal(s) with "
				...
				try {
					...
					for (int i = 0; i < numTerminals; i++) {
						...
						} while (usedTerminals[terminalWarehouseID
								- 1][terminalDistrictID - 1] == 1);
						...
						jTPCCTerminal terminal = new jTPCCTerminal(terminalName,
								terminalWarehouseID, terminalDistrictID, conn,
								dbType, transactionsPerTerminal,
					...
					printMessage(
							"All terminals started executing " + sessionStart);
				}
	...
	public void signalTerminalEndedTransaction(String terminalName,
		...
		synchronized (counterLock) {
			...
			if (counter == null) {
				costPerWorkerload.put(transactionType,
	...
	public void resultAppend(jTPCCTData term) {
		if (resultCSV != null) {
			try {
				resultCSV.write(
						runID + "," + term.resultLine(sessionStartTimestamp));
			} catch (IOException e) {
	...
	private void endReport() {
		...
		double tpmC = (6000000 * fastNewOrderCounter
				/ (currTimeMillis - sessionStartTimestamp)) / 100.0;
