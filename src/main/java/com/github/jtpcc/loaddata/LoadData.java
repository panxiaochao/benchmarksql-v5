...
public class LoadData {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.err.println(
					"ERROR: cannot load JDBC driver - " + e.getMessage());
			System.exit(1);
		}
		...
		if (fileLocation != null) {
			...
			try {
				configCSV = new BufferedWriter(
						new FileWriter(fileLocation + "bmsql_config.csv"));
				itemCSV = new BufferedWriter(
						new FileWriter(fileLocation + "bmsql_item.csv"));
				warehouseCSV = new BufferedWriter(
						new FileWriter(fileLocation + "bmsql_warehouse.csv"));
				districtCSV = new BufferedWriter(
						new FileWriter(fileLocation + "bmsql_district.csv"));
				stockCSV = new BufferedWriter(
						new FileWriter(fileLocation + "bmsql_stock.csv"));
				customerCSV = new BufferedWriter(
						new FileWriter(fileLocation + "bmsql_customer.csv"));
				historyCSV = new BufferedWriter(
						new FileWriter(fileLocation + "bmsql_history.csv"));
				orderCSV = new BufferedWriter(
						new FileWriter(fileLocation + "bmsql_oorder.csv"));
				orderLineCSV = new BufferedWriter(
						new FileWriter(fileLocation + "bmsql_order_line.csv"));
				newOrderCSV = new BufferedWriter(
						new FileWriter(fileLocation + "bmsql_new_order.csv"));
	...
	private static String iniGetString(String name, String defVal) {
		...
		if (strVal == null) {
			System.out.println(
					name + " (not defined - using default '" + defVal + "')");
			return defVal;
		} else if (name.equals("password"))
			System.out.println(name + "=***********");
		else
