...
import com.github.jtpcc.client.jTPCCRandom;
...
public class LoadDataWorker implements Runnable {
	...
	LoadDataWorker(int worker, Connection dbConn, jTPCCRandom rnd)
		...
		stmtItem = dbConn.prepareStatement("INSERT INTO bmsql_item ("
				...
				+ "VALUES (?, ?, ?, ?, ?)");
		stmtWarehouse = dbConn.prepareStatement("INSERT INTO bmsql_warehouse ("
				+ "  w_id, w_ytd, w_tax, w_name, w_street_1, w_street_2, w_city, "
				+ "  w_state, w_zip) " + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
		...
		stmtOrder = dbConn.prepareStatement("INSERT INTO bmsql_oorder ("
				...
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
		stmtOrderLine = dbConn.prepareStatement("INSERT INTO bmsql_order_line ("
		...
		try {
			while ((job = LoadData.getNextJob()) >= 0) {
				if (job == 0) {
					...
					fmt.format("Worker %03d: Loading Warehouse %6d", worker,
							job);
					System.out.println(sb.toString());
					sb.setLength(0);
	...
	private void loadWarehouse(int w_id) throws SQLException, IOException {
		...
		if (writeCSV) {
			fmtWarehouse.format("%d,%.2f,%.4f,%s,%s,%s,%s,%s,%s\n", w_id,
					300000.0, ((double) rnd.nextLong(0, 2000)) / 10000.0,
					rnd.getAString(6, 10), rnd.getAString(10, 20),
					rnd.getAString(10, 20), rnd.getAString(10, 20),
					rnd.getState(), rnd.getNString(4, 4) + "11111");

			LoadData.warehouseAppend(sbWarehouse);
		} else {
			stmtWarehouse.setInt(1, w_id);
			stmtWarehouse.setDouble(2, 300000.0);
			stmtWarehouse.setDouble(3,
		...
		for (int s_i_id = 1; s_i_id <= 100000; s_i_id++) {
			...
			if (writeCSV) {
				fmtStock.format(
						"%d,%d,%d,%d,%d,%d,%s,"
								+ "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n",
						w_id, s_i_id, rnd.nextInt(10, 100), 0, 0, 0, sData,
						rnd.getAString(24, 24), rnd.getAString(24, 24),
						rnd.getAString(24, 24), rnd.getAString(24, 24),
						rnd.getAString(24, 24), rnd.getAString(24, 24),
						rnd.getAString(24, 24), rnd.getAString(24, 24),
						rnd.getAString(24, 24), rnd.getAString(24, 24));
		...
		for (int d_id = 1; d_id <= 10; d_id++) {
			if (writeCSV) {
				fmtDistrict.format("%d,%d,%.2f,%.4f,%d,%s,%s,%s,%s,%s,%s\n",
						w_id, d_id, 30000.0,
						((double) rnd.nextLong(0, 2000)) / 10000.0, 3001,
						rnd.getAString(6, 10), rnd.getAString(10, 20),
						rnd.getAString(10, 20), rnd.getAString(10, 20),
						rnd.getState(), rnd.getNString(4, 4) + "11111");
			...
			for (int c_id = 1; c_id <= 3000; c_id++) {
				...
				if (writeCSV) {
					fmtCustomer.format(
							...
							(c_id <= 1000)
									? rnd.getCLast(c_id - 1)
									: rnd.getCLast(),
							rnd.getAString(8, 16), 50000.00, -10.00, 10.00, 1,
							1, rnd.getAString(10, 20), rnd.getAString(10, 20),
							rnd.getAString(10, 20), rnd.getState(),
							rnd.getNString(4, 4) + "11111",
							rnd.getNString(16, 16),
							new java.sql.Timestamp(System.currentTimeMillis())
									.toString(),
							"OE", rnd.getAString(300, 500));
				} else {
					stmtCustomer.setInt(1, w_id);
					stmtCustomer.setInt(2, d_id);
					stmtCustomer.setInt(3, c_id);
					stmtCustomer.setDouble(4,
							((double) rnd.nextLong(0, 5000)) / 10000.0);
				...
				if (writeCSV) {
					fmtHistory.format("%d,%d,%d,%d,%d,%d,%s,%.2f,%s\n",
							(w_id - 1) * 30000 + (d_id - 1) * 3000 + c_id, c_id,
							d_id, w_id, d_id, w_id,
							new java.sql.Timestamp(System.currentTimeMillis())
									.toString(),
							10.00, rnd.getAString(12, 24));
				} else {
					...
					stmtHistory.setTimestamp(6,
							new java.sql.Timestamp(System.currentTimeMillis()));
					stmtHistory.setDouble(7, 10.00);
					stmtHistory.setString(8, rnd.getAString(12, 24));
			 ...
			 * generated using a random permutation of all 3,000 customers. To
			 * do that we set up an array with all C_IDs and then randomly
			 * shuffle it.
			 */
				...
				for (int ol_number = 1; ol_number <= o_ol_cnt; ol_number++) {
					...
					if (writeCSV) {
						fmtOrderLine.format("%d,%d,%d,%d,%d,%s,%.2f,%d,%d,%s\n",
								...
								(o_id < 2101)
										? 0.00
										: ((double) rnd.nextLong(1, 999999))
												/ 100.0,
								w_id, 5, rnd.getAString(24, 24));
					} else {
