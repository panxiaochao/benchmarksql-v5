...
public class LoadDataWorker implements Runnable {
  ...
  LoadDataWorker(int worker, Connection dbConn, jTPCCRandom rnd) throws SQLException {
    ...
    stmtOrderLine = dbConn.prepareStatement(
        "INSERT INTO bmsql_order_line (" + "  ol_w_id, ol_d_id, ol_o_id, ol_number, ol_i_id, "
  ...
  private void loadWarehouse(int w_id) throws SQLException, IOException {
    ...
    if (writeCSV) {
      fmtWarehouse.format("%d,%.2f,%.4f,%s,%s,%s,%s,%s,%s\n", w_id, 300000.0,
          ((double) rnd.nextLong(0, 2000)) / 10000.0, rnd.getAString(6, 10), rnd.getAString(10, 20),
          rnd.getAString(10, 20), rnd.getAString(10, 20), rnd.getState(),
    ...
    for (int s_i_id = 1; s_i_id <= 100000; s_i_id++) {
      ...
      if (writeCSV) {
        fmtStock.format("%d,%d,%d,%d,%d,%d,%s," + "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n", w_id, s_i_id,
            rnd.nextInt(10, 100), 0, 0, 0, sData, rnd.getAString(24, 24), rnd.getAString(24, 24),
            rnd.getAString(24, 24), rnd.getAString(24, 24), rnd.getAString(24, 24),
            rnd.getAString(24, 24), rnd.getAString(24, 24), rnd.getAString(24, 24),
    ...
    for (int d_id = 1; d_id <= 10; d_id++) {
      if (writeCSV) {
        fmtDistrict.format("%d,%d,%.2f,%.4f,%d,%s,%s,%s,%s,%s,%s\n", w_id, d_id, 30000.0,
            ((double) rnd.nextLong(0, 2000)) / 10000.0, 3001, rnd.getAString(6, 10),
            rnd.getAString(10, 20), rnd.getAString(10, 20), rnd.getAString(10, 20), rnd.getState(),
            rnd.getNString(4, 4) + "11111");
      ...
      for (int c_id = 1; c_id <= 3000; c_id++) {
        ...
        if (writeCSV) {
          fmtCustomer.format(
              ...
              w_id, d_id, c_id, ((double) rnd.nextLong(0, 5000)) / 10000.0,
              (rnd.nextInt(1, 100) <= 90) ? "GC" : "BC",
              (c_id <= 1000) ? rnd.getCLast(c_id - 1) : rnd.getCLast(), rnd.getAString(8, 16),
              50000.00, -10.00, 10.00, 1, 1, rnd.getAString(10, 20), rnd.getAString(10, 20),
              rnd.getAString(10, 20), rnd.getState(), rnd.getNString(4, 4) + "11111",
      ...
      for (int o_id = 1; o_id <= 3000; o_id++) {
        ...
        if (writeCSV) {
          fmtOrder.format("%d,%d,%d,%d,%s,%d,%d,%s\n", w_id, d_id, o_id, randomCID[o_id - 1],
